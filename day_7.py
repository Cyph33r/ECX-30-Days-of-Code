...
- The user makes a first guess..
- Then you get a feedback looking like "√××√+".
 * "√" Indicates that the letter is in the secret word and in the correct position.
 * "+" indicates that the letter is in the hidden word but in a different position.
 * "×" indicates that the letter is not in the word
- The player is given a total of 6 guesses in which he must guess the secret word.'''
print(greetings)
...
while round_num <= 6:
    ...
    if feedback.count('√') == 5:  # check if the player guessed the word successfully
        print(f"Congrats. You guessed the word after {round_num} attempt/s")
        break
